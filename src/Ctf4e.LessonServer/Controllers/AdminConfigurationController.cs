...
namespace Ctf4e.LessonServer.Controllers
{
    ...
    {
        private readonly IOptionsSnapshot<LessonOptions> _lessonOptions;
        private readonly ILessonConfigurationService _lessonConfiguration;
        private readonly IStateService _stateService;

        public AdminConfigurationController(IOptionsSnapshot<LessonOptions> lessonOptions, ILessonConfigurationService lessonConfiguration, IStateService stateService)
            : base("~/Views/AdminConfiguration.cshtml", lessonOptions, lessonConfiguration)
        {
            _lessonOptions = lessonOptions;
        ...
        private async Task<IActionResult> RenderAsync(string configuration = null)
        {
            try
            {
                ...
                var configFile = new FileInfo(_lessonOptions.Value.LessonConfigurationFile);
        ...
        public async Task<IActionResult> UpdateConfigurationAsync(string configuration)
        {
            ...
            try
            {
                var newOptions = JsonConvert.DeserializeObject<LessonConfiguration>(configuration);
            ...
            try
            {
                ...
                await System.IO.File.WriteAllTextAsync(_lessonOptions.Value.LessonConfigurationFile, configuration);
                
                // Reload
                await _lessonConfiguration.ReadConfigurationAsync();
