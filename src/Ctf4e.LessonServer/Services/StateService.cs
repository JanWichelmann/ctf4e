...
namespace Ctf4e.LessonServer.Services
{
    ...
    /// <summary>
    /// Contains and manages the lesson state.
    /// </summary>
    public class StateService : IStateService, IDisposable
    {
        private readonly IOptions<LessonOptions> _options;
        ...
        private SortedDictionary<int, LessonConfigurationExerciseEntry> _exercises;
        ...
        public StateService(IOptions<LessonOptions> options, ILessonConfigurationService lessonConfiguration, IDockerService dockerService)
        {
        ...
        public void Reload()
        {
            ...
            // Read exercises
            _exercises = new SortedDictionary<int, LessonConfigurationExerciseEntry>(_lessonConfiguration.CurrentConfiguration.Exercises.ToDictionary(e => e.Id));
            ...
            foreach(string userStateFileName in Directory.GetFiles(_options.Value.UserStateDirectory, "u*.json"))
            {
                ...
                {
                    if(exercise.Value is LessonConfigurationStringExerciseEntry stringExercise)
                    {
                    ...
                    else if(exercise.Value is LessonConfigurationMultipleChoiceExerciseEntry multipleChoiceExercise)
                    {
                    ...
                    else if(exercise.Value is LessonConfigurationScriptExerciseEntry scriptExercise)
                    {
